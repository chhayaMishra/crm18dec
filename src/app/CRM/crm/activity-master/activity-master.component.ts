...
export class ActivityMasterComponent implements OnInit {
  ...
  CurrencyId: any;
  // lead status
  leadStatus =[{"statusId":1,"statusName":"New Lead" , "BackgroundColor":"rgb(120, 148, 209)","color": "white", "margin": "10px"},
  {"statusId":2,"statusName":"Attempted to contact", "BackgroundColor":"rgb(193, 148, 252)","color": "white", "margin": "10px"},
  {"statusId":3,"statusName":"Contacted", "BackgroundColor":"rgb(252, 148, 200)","color": "white", "margin": "10px"},
  {"statusId":4,"statusName":"Closed", "BackgroundColor":"rgb(90, 77, 206)","color": "white", "margin": "10px"}]
  id: any;
 ...
 console.log(this.oppoId)
  ...
  getallactivity() {
    this.crmService.getallactivity().subscribe((res: any) => {
      ...
      this.ActivityData.forEach((activity) => console.log(`notes: ${activity.Notes}`));
      this.ActivityData.forEach((activity) =>
        console.log(`start: ${activity.StartTime}`)
      );
      this.ActivityData.forEach((activity) => console.log(`end: ${activity.EndTime}`));
  ...
  getactivitybylead(leadId) {
    ...
    this.crmService.getactivitybylead(leadId).subscribe((res: any) => {
      ...
      this.ActivityData.forEach((activity) => console.log(`leaid: ${activity.LeadId}`));
  ...
  openAddActivityDialog(): void {
    const dialogRef = this.dialog.open(AddActivityDialogComponent, {
      width: '300px',
      data: { activityName: '', activityDate: '' } // Initial data for the dialog
    });
